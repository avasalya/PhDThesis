...
\section{Multi-Contact Bi-directional Handover - \textit{using hands together}}\label{both hands together}
...
Under handover scenarios, we initiate handover routine when human holds the object (either with left or right or both hands) and start approaching somewhere within the allowable workspace of robot, assuming that he/she is ready with the intention to handover the object to robot. Human can hold the object with any possible comfortable orientation using either left/right or both hands simultaneously and any place on the object surface along its length. The only constraint we put while holding/grasping object was not to occlude the $\mathcal{L}$ shape body and mocap markers on it (see Fig. (XXX - get video frame) for such possible poses).

\paragraph*{Before contacts:}
Compared to single hand handover scenario which was discussed in previous Section~\ref{both hands individual}, here during human to robot object handover \textit{cycle} instead of just predicting human hand positions individually like earlier, we also estimate the relative position of human hands with respect to object's center using equation XX. That relative position acts as an offset correction to the predicted position for robot end-effectors. This offset is important as it allows robot to find safe and optimum position on the object surface to grasp which is quite away from the human hands occupied object surface. Also it avoids any possible head-on collision between end-effectors and human hands when robot tries to approach and grasp the object.

\begin{equation}
	content...
\end{equation}

To predict and estimate human hand positions, we utilized previously discussed prediction model from Section~\ref{prediction_model}. Given object shape and its structural properties (in our case length of cylinder shaft), the offset can be calculated using equation XX in the object local frame $o$ (also see Fig. XX pipe offset fig) from the center in both directions for each end-effectors. This offset is then further transform into world frame using equation XX and finally added to the already predicted position of human hands with respect to robot end-effectors.
...
\paragraph*{After contacts}
we Predefined contact surfaces in advance, 
