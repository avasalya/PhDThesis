...
\begin{tikzpicture}[remember picture, overlay, shift=(current page.north west)]
	...
	\\ \Large \textbf{HRI to physical-HRI} 
...
However, this thesis partially focues on the behavioural effect of those human skills learned humanoid robots on their human co-worker during a common task and rest of thesis deals with study of physical interaction between human and robot during an interactive task of object handover.
...
\begin{figure}[hpt]
		\centering{\includegraphics[width=0.7\columnwidth]{plots/c4-plots/Fig8}}
...
chapter abstract comes here


\clearpage
\section{Introduction}

Handover is naturally an explicit yet complex interactive collaborative task that occurs seamlessly multiple times between humans on a daily basis and under several scenarios.
...
\section{Whole-body control based handover}
...
\textit{The robot needs to accurately predict this point of transfer to reach out proactively, instead of waiting for the final position to be presented}
...
we take similar approach as one mentioned in~\cite{medina2016human} and treat handover as one stage cycle to perform intuitive and responsive handover between human and humanoid robot co-worker.
...
This section focuses on simplified explanation of bi-directional handover model using basic block diagram structure as shown in Fig~\ref{fig:handover routine}.

We started handover routine to handover object using human right hand to robot left end-effector, hereafter until before Section~\nameref{both hands individual} we will explain the everything under this condition, afterwards we will talk about generalization of handover using the possible handover combination scenarios. Such as 
\begin{itemize}
...
talk about {environment} lab setup
...
\paragraph*{}
In order to correctly transform desired human hand orientation into robot end-effector frame, we further utilize the equation (\ref{X_ef_ht}). We take advantage of the QP's Orientation Task~\cite{murray2017mathematical, ladder-HRP-2Kai} (see Section~\nameref{qpTasks}) and compute the task error $Err$ between \textit{current} human hand orientation and \textit{fixed} robot end-effector orientation, the resulting orientation from the Orientation Task is the desired robot end-effector orientation relative to the human hand orientation (see Fig~\ref{fig:robot_lt_orientations} for few of many possible orientations), therefore using fixed orientation component  ${{}^{efInit}\mathcal{O}_R}$, of equation (\ref{X_R_ef}) and current human hand orientation ${}^{h}\mathcal{O}_M$ from equation (\ref{rotationmatrix}) derived in Section~\nameref{hand_orientation}, we further modify equation (\ref{X_ef_ht}) to get the desired orientation $ {}^{h}\mathcal{O}_{ef} $ of robot end-effector to handover the object.
...
We explained further in detail in the Subsection~\nameref{FSM}, but before that we mention another method~\ref{surface wrench} that we also utilize for closing gripper when object is in the vicinity.
...
\begin{enumerate}[start=0,label={\bf{t}\arabic*:}]
    ...
    ${}^{obj}\overline{\mathcal{F}}$, is pure force sensor reading when ${}^{ef}a=0$ and object is grasped by the robot, $n$ is the low-level controller time-step counter increments every $5$ms, see Section~\nameref{qpTasks}, we intentionally stay on this state for 1 second i.e. until $i==200$ to measure average of ${}^{obj}\vec{\mathcal{F}}$ over 200 samples. Object $mass$ can be obtained using equation (\ref{obj mass}).
...
\begin{cases}
	...
	{}^\text{off}{d} = \pm a/2 &  \text{if} \; a \geq b \; \bigwedge \; a\leq l/2\\

	{}^\text{off}{d} = \mp b/2 &  \text{if} \; a < b \; \bigwedge \; a\leq l/2\\
\end{cases}
\end{equation}
...
\paragraph*{}
Handover occurs when set of following conditions in equation (\ref{min_posErr_vel}) satisfy for both robot end-effectors, see Section~\nameref{prediction_model} for more information. We add contact constraints between robot left and right gripper surfaces and object surface after robot grasp the object using both end-effectors. 

12.2 Constrained Dynamics
As soon as the manipulator comes in contact with the
environment, say a rigid surface as shown in Figure 12.2, one or more degrees of freedom in motion may be lost since the manipulator cannot move through the environment surface.


We Predefined contact surfaces in advance, as shown in Fig.~\ref{fig:gripper-intern}. These contacts are necessary to move freely or limit motion in one or more direction (translation or rotation).

A DoF contact, short for Degree of Freedom contact, is a facility that allows us to simulate sliding contacts. To be more precise, whenever we have a contact, we can free movement along one or more axis (in translation and/or rotation).
...
\begin{figure}[hptb]
	\centering{\includegraphics[width=1\columnwidth]{plots/c5-plots/grippers-intern}}
...
%refer chapter 11 of \cite{featherstone2014rigid} constraint motion, look for velocity constraint,8.2 Loop Constraint Equations

%Tasks::qp::ContactSpeedConstr(timeStep) QPContactConstr.h/cpp
%$https://en.wikipedia.org/wiki/Holonomic_constraints$


file:///home/avasalya/Dropbox%20(CNRS-AIST%20JRL)/PhD/Book%20&%20ppt/multi%20contact%20planning%20and%20control.pdf
...
on the friction cones that we consider for regular contact
