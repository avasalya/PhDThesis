...
\begin{algorithm}[H]\label{interaction forces}
	...
	\If{\text{human hand is near robot}}
	{
		...
		${}^\text{obj}\overline{\mathcal{F}} = \frac{1}{n}\sum_{i=1}^{i=n} \vert{ (\vert{\mathcal{F}} - \vert{{}^\text{zero}\mathcal{F}}) }$
		...
		{	
			${}^\text{pull}\mathcal{F}$ $= \vert{(\vert\mathcal{{F}} - \vert{}^\text{inert}\mathcal{{F}} - \vert{}^\text{zero}\mathcal{{F}}) }$
