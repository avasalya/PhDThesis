...
\begin{figure}[htbp]
		\centering{\includegraphics[width=0.6\columnwidth]{plots/c4-plots/halfsit}}
...
In Fig.~\ref{fig:handover routine}, we describe the process of our bi-directional handover routine using a continuous yet step-by-step approach. We started the handover routine to handover the object using human right hand to robot left end-effector, from hereafter until and before Section~\nameref{both hands individual} we will explain everything under this condition, afterwards we will talk about generalization of handover using possible handover combination scenarios. Such as,
...
%(more information on mocap streaming can be found in Appendix~\ref{handover appendix})
...
\begin{figure}[htpb]
	\centering{\includegraphics[width=0.6\columnwidth]{plots/c4-plots/objects}}
...
\begin{figure}[ht]
	\centering{\includegraphics[width=0.8\columnwidth]{plots/c4-plots/lshpe-lt-rt}}
...
where, ${}^{h}\mathcal{P}_{ef}(t_{predict})$ is the desired position for robot end-effector to reach and ${}^{ef}\mathcal{P}_{R}$ is the actual current position of robot end-effector and also both positions are in same frame or transformed otherwise.
...
\begin{figure}[ht]
	\centering{\includegraphics[width=.8\linewidth]{plots/c4-plots/efmarkers.png}}
...
% 	\textit{$\textbf{dt++}$} \tcp*{increments as per controller run-time (5ms)}
	...
	\If{\text{human hand is near robot}}
	{
		...
		$objectMass = \norm{{}^{obj}\overline{\mathcal{F}}}/9.8 $\tcp*{get object mass}
...
To the best of our knowledge, bi-directional object handover with biped walking have not been considered in the previous works on the human-robot dyad object handover. 
 
To the best of our knowledge, none of the previous work considered object handover and `walking' in a single framework using a \textit{biped} humanoid robot such as HRP-2Kai.
