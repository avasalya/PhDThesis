...
%also as per strength to weight ratio (cite XXX) hollow cylinder is stronger than solid rod for equal mass and material.
...
Following is the work flow of both hands handover motion.
\begin{enumerate}
	\item human has object.
	\item both robot's end-effectors approaches towards the object.
	\item robot receives the object.
	\item contacts are established between robot grippers and object surface.
	\item one of the robot's end-effector position and orientation tasks are removed.
	\item other robot's end-effector follows the dominant human hand.
\end{enumerate}
...
where, ${}^{o}\mathcal{O}_M$ provides the orientation of object in the world frame (see equation~\ref{X_M_o}) and ${}^{h}\mathcal{P}_M$ is the predicted position of human hand(s). Finally, using equation (\ref{offset_transform}), we can replace translation and orientation components of ${}^{h}{X}_M$ in the equation (\ref{X_ef_ht}) with ${}^\text{off}{X}_{M}$, therefore new pose of the handover location relative to the robot end-effectors would be given by
...
\paragraph*{}
Handover occurs when set of following conditions in equation (\ref{min_posErr_vel}) satisfy for both robot end-effectors, see Section~\nameref{prediction_model} for more information. 
...
\paragraph*{After object-robot contacts:}{\bf  We initially defined} a regular contact surface on the palm of each gripper in advance, as shown in Fig.~\ref{fig:gripper-intern}. After the handover when the robot grasp the object and its grippers internal surface come in contact with the object's graspable surface, robot may lose one or more degrees of freedom in motion due to the constraints introduced by the object's body. These contacts are necessary to move freely or limit the motion of end-effectors in one or more direction (translation or rotation)~\cite{bouyarmane2018multi}. After the contacts are established, the object body starts to act as an extension to robot's own body due to the kinematic constraint between them. 


Lets consider two

\paragraph*{}
Up till now, each robot end-effector was controlled by its own \texttt{position} and \texttt{orientation} task. However, once robot holds the object using both end-effectors, the successive motion of the end-effectors is now governed by the \texttt{contactVelConstr} task that we introduce to the robot's low-level QP controller. This task maintains the contact constraint between object and robot gripper by targeting a null velocity, see Subsection~\nameref{QPTasks} for further explanation.

% 3.5 joint constraint- Featherstone
\begin{equation}
...
So far we have discussed estimating and predicting handover pose for the robot end-effectors during 1${}^{st}$ \textit{cycle} of object handover routine. During the 2$^{nd}$ \textit{cycle} of handover after the contacts have been established between robot grippers and object. 
...
\begin{figure}[ht]
	\centering{\includegraphics[width=0.8\columnwidth]{plots/c5-plots/wrist-surface}}
	\caption{Virtual intrinsic surfaces (green) on robot wrists.}
...
\section{Step-walk and handover}
