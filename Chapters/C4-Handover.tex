...
%Handover is naturally an explicit yet simple  physical interactive task that occurs seamlessly multiple times between humans on a daily basis and under several scenarios.
%
%
%although handovers are fluent phase-less natural events between human human interactions, but during human robot object handovers, many previous works (XXXX) have treated handover routine as non-continuous entity divided into 3 sub-tasks — approach, indication, deliver~\cite{kupcsik2016learning, strabala2013toward}, many previous studies have ha
% 
%
%question to answer ---  when(timing), where (pos), how(orientation and forces),
% 
%--- grasp planning- a process that determines the most appropriate object and then the most appropriate grasp for executing the given task.
%
...
%
%
%\textit{In future, robots will be present in everyday life. The development of these supporting robots is a challenge. A fundamental task for assistance robots is to pick up and hand over objects to humans. By interacting with users, soft factors such as predictability, safety and reliability become important factors for development. Previous works show that collaboration with robots is more acceptable when robots behave and move human-like}. 
%
%
%
%Three important key features that we want to focus during human humanoid robot object handover is the \textit{timing} of handover and \textit{pose} of handover and \textit{magnitude} of interaction forces between human and humanoid robot end-effector. Basically we would like to answer following questions ---when(\textit{timing}), where (\textit{position}), how(\textit{orientation and interaction forces}).\\
%
%In case of handover, the estimation of handover time as well as the exact position of handover must be realized in order to bring the hand at appropriate place and approximately on appropriate time ---to be ready to receive the tool.\\
%The robot needs to accurately predict this point of transfer to reach out proactively, instead of waiting for the final position to be presented\\
%--- when to start the motion? -- relative distance -- hysteresis\\
%--- how to meet at handover location ? -- pose estimation\\
%--- what are the precepts on which the robot must relay to release the tool?  --- handover interaction model\\
%--- robot workspace -- field of view
%
%
...
%
%
%Many position prediction methods have already been explored in the past like~\cite{li2015predicting} discussed several position prediction model, however since position prediction is not the focus of our study, therfore, we primarily decided to go with most common method i.e.\textit{constant velocity} based model, see Section~\nameref{prediction_model} for details.


\subsection{Whole-body control based handover}
...
%
%
%Our whole-body handover planner is model-based. It needs the models of the HRP-2Kai, the pertinent information of the environment (object), mainly handover object's shape and size information, though knowing mass of the object is not important in the beginning.
%
%\textbf{
%}\\
%
%
%-- two broad categories of handover motion
...
\begin{figure}[htbp]
		\centering{\includegraphics[width=1\columnwidth]{plots/c5-plots/halfsit}}
...
\begin{figure}[htpb]
	\centering{\includegraphics[width=0.8\columnwidth]{plots/c5-plots/objects}}
...
\item ContactConstraint task: $\mathscr{T}_{contact} = J_i(q)\ddot{q}_{s(k)} + \dot{J}(q)_i\dot{q}_{p(k)} = 0$\\
The target objective of this task is to maintain a null velocity between the joints of bodies in contact, where $p(k)$ and $s(k)$ are the predecessor and successor bodies of robot(s) in contact that imposes a constraint~\cite{featherstone2014rigid}.\\
...
\begin{figure}[ht]
	\centering{\includegraphics[width=0.5\columnwidth]{plots/c5-plots/frame}}
...
\begin{figure}[ht]
	\centering{\includegraphics[width=\linewidth]{plots/c5-plots/efmarkers.png}}
...
\begin{figure}[ht]
	\centering{\includegraphics[width=0.8\columnwidth]{plots/c5-plots/wrist-surface2}}
...
\begin{figure}[hpt]
	\centering{\includegraphics[width=1\columnwidth]{plots/c5-plots/hr-rr}}
...
\begin{figure}[hpt]
	\centering{\includegraphics[width=1\columnwidth]{plots/c5-plots/handoverPipe}}
...
\begin{figure}[hpt]
	\centering{\includegraphics[width=1\columnwidth]{plots/c5-plots/robot_has_pipe}}
...
Here we approached differently during the 1${}^{st}$ and 2${}^{nd}$ \textit{cycle}s of handover. In 1${}^{st}$ \textit{cycle} of object handover from human to humanoid robot, we predict and estimate both human hands pose individually. Specifically, we used unique \texttt{position} and \texttt{orientation} tasks (see Subsection~\nameref{QPTasks}) to move the robot end-effectors to the desired handover location. 

But in the 2${}^{nd}$ \textit{cycle}, when the robot hold the object using both end-effectors, and due to the introduction of motion constraints by the object. it is not possible to move 


we only predict and estimate pose of dominant human hand. Here only the robot end-effector relative to the dominant human hand stays in the QP controller and in addition we introduced \texttt{contactConstraint} task. Following is an overview of both hands handover motion.
...
\begin{compactitem}
	...
	\item Contacts are established between robot grippers and object surface.
...
\begin{figure}[hptb]
	\centering{\includegraphics[width=1\columnwidth]{plots/c5-plots/gripperContact}}
	\caption{Robot grippers with graspable internal surfaces. remove B fig already above}
	\label{fig:gripper-intern}
\end{figure}


\paragraph*{Contact surface(s)} 
We initially defined a regular contact surface on the palm of each gripper in advance, as shown in Fig.~\ref{fig:gripper-intern} and Fig.~\ref{fig:wrist-surface}. 

After the handover when the robot grasp the object and its grippers internal surface come in contact with the object's graspable surface. 

Robot may lose one or more degrees of freedom in motion due to the constraints introduced by the object's body.

These contacts are necessary to move freely or limit the motion of end-effectors in one or more direction (translation or rotation)~\cite{bouyarmane2018multi}. 

After the contacts are established, the object body starts to act as an extension to robot's own body due to the kinematic constraint between them. 

\paragraph*{Contact type and constraint}
Contacts are defined as kinematic constraints in our robot's native QP controller, specifically we used the fixed contact mode, as defined by the~\cite{balkcom2002computing}, where all six degrees of freedom are constrained (no translation, no rotation at contact) which restricts and prevents any possibility of sliding motion between the robot end-effectors and the object.

\paragraph*{Contact kinematics}
Lets consider $b_{efl}$ $b_{efr}$ and $b_o$ be the left, right end-effectors and object body links.

velocity constraint \cite{ohwovoriole1980externsion}


\begin{figure}[hptb]
	\centering{\includegraphics[width=0.8\columnwidth]{plots/c5-plots/h-r-pipe-handover}}
...
%\begin{figure}[hptb]
%	\centering{\includegraphics[width=1\columnwidth]{plots/c5-plots/pipe-offset}}
...
Handover occurs when the set of following conditions in equation (\ref{min_posErr_vel}) satisfy for both robot end-effectors, see Section~\nameref{prediction_model} for more information. 


\paragraph*{$2^{nd}$ \textit{cycle}: after object-robot contacts} 
So far we have discussed estimating and predicting handover pose for the robot end-effectors during 1${}^{st}$ \textit{cycle} of object handover routine. During the 2$^{nd}$ \textit{cycle} of handover after the contacts have been established between robot grippers and object. Up till now, each robot end-effector was controlled by its own \texttt{position} and \texttt{orientation} task. However, once robot holds the object using both end-effectors, the successive motion of the end-effectors is then governed by the \texttt{contactConstraint} task that we introduce to the robot's low-level QP controller. This task maintains the contact constraint between object and robot gripper by targeting a null velocity, see Subsection~\nameref{QPTasks} for further explanation. 


we remove the null velocity constraint task whenever $f_{surf}$ along the gripper's insertion ($z$)-axis of surface wrench exceeds 2N, this condition is valid only if human tries to pull the object from the robot.


% 3.5 joint constraint- Featherstone, also page 229 onwards
%\begin{equation}
%\mathscr{T}_{contact} = J_i(q)\ddot{q}_{s(k)} + \dot{J}(q)_i\dot{q}_{p(k)} = 0
%\end{equation}

therefore, to govern


%Finally we measure noise free ${}^{surf}\vec{f}(y)$ force along the local $y$-axis of surface frame $surf$ (see Fig.~\ref{fig:wrist-surface}) during the interaction between object and gripper just prior to handover and if the measured force $\norm{{}^{surf}\vec{f}(y)}$ is greater than 2N along with the outcome of equation (\ref{area}) then it is safe for robot to close the gripper and hence grasp the object.
%
%
...
\begin{figure}[ht]
	\centering{\includegraphics[width=0.8\columnwidth]{plots/c5-plots/handoverpipeIntern}}
...
refer \url{file:///home/avasalya/Dropbox%20(CNRS-AIST%20JRL)/PhD/Papers/other's%20thesis/thesis-antonio-bussy.pdf}
	for parameters to analyze
